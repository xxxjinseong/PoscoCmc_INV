  ...
  try
    ...
    begin
      ...
      StrSQL := ' SELECT ' +
                ...
                ' CASE WHEN RTRIM(LTRIM(S.BODY_01)) = ''R'' THEN ''이동실적'' ' +        // 실적구분
                '      WHEN RTRIM(LTRIM(S.BODY_01)) = ''C'' THEN ''실적취소'' ' +
                '      ELSE '''' END as BODY_01, ' +
                ' ''완료'' as PROC_MANUAL, ' +                                    // 완료상태
                ' CONVERT([varchar](19), CONVERT(DATETIME, (SUBSTRING(RTRIM(LTRIM(S.BODY_02)), 1, 8) + '' '' + ' +
                ' STUFF(STUFF(SUBSTRING(RTRIM(LTRIM(S.BODY_02)),9, 6), 3, 0, '':''), 6, 0, '':'')), 21), 21) as BODY_02, ' + // 이동일시
                ' RTRIM(LTRIM(S.BODY_03)) as BODY_03, ' +                                // 제품번호
                ' RTRIM(LTRIM(S.BODY_06)) as BODY_06, ' +                                // 제품코드
                ' RTRIM(LTRIM(S.BODY_07)) as BODY_07, ' +                                // 제품명
                ' CASE WHEN RTRIM(LTRIM(S.BODY_08)) = '''' or RTRIM(LTRIM(S.BODY_08)) = NULL THEN CONVERT(NUMERIC(9,3), 0) ' +
                '      ELSE CONVERT(NUMERIC(9,3), RTRIM(LTRIM(S.BODY_08))) END as BODY_08, ' +  // 중량
                ' RTRIM(LTRIM(S.BODY_09)) as BODY_09, ' +                                // 고객코드
                ' RTRIM(LTRIM(S.BODY_10)) as BODY_10, ' +                                // 고객명
                ' RTRIM(LTRIM(S.BODY_11)) as BODY_11, ' +                                // 주문번호
                ' RTRIM(LTRIM(S.BODY_12)) as BODY_12, ' +                                // 제품여부
                // 이동 전 적치위치
                ...
                // 적치호기
                ' CASE WHEN SUBSTRING(RTRIM(LTRIM(S.BODY_05)), 1, 2) = ''01'' or SUBSTRING(RTRIM(LTRIM(S.BODY_05)), 1, 2) = ''02'' THEN ''1호기'' ' +
                '      WHEN SUBSTRING(RTRIM(LTRIM(S.BODY_05)), 1, 2) = ''03'' or SUBSTRING(RTRIM(LTRIM(S.BODY_05)), 1, 2) = ''04'' THEN ''2호기'' ' +
                '      WHEN SUBSTRING(RTRIM(LTRIM(S.BODY_05)), 1, 2) = ''05'' or SUBSTRING(RTRIM(LTRIM(S.BODY_05)), 1, 2) = ''06'' THEN ''3호기'' ' +
                '      ELSE '''' END as HOGI, ' +
                // 전송여부
