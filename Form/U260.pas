...
begin
  try
    ...
    begin
      ...
      TraceLogWrite( '[260] 자동창고 혼재랙정리 에러 | ' +
...
//==============================================================================
// fnCommandQueryOrderCnt [호기별 진행 작업 개수]
//==============================================================================
  ...
  try
    ...
    begin
      ...
      StrSQL := ' SELECT SC_HOGI, ' +
                ...
                '           UNION ALL ' +
                '          SELECT 0 AS SC_HOGI, ' +
                '                 CASE WHEN IO_STAT = ''입고'' THEN 1 ELSE 0 END AS JOB_CNT1, ' +
                '                 0 as JOB_CNT2, ' +
                '                 0 as JOB_CNT3 ' +
                ...
                '          SELECT 1 AS SC_HOGI, ' +
                '                 CASE WHEN IO_STAT = ''입고'' THEN 1 ELSE 0 END AS JOB_CNT1, ' +
                '                 CASE WHEN IO_STAT = ''출고'' THEN 1 ELSE 0 END AS JOB_CNT2, ' +
	              '                 CASE WHEN IO_STAT = ''이동'' THEN 1 ELSE 0 END AS JOB_CNT3  ' +
                '            FROM TT_ORDER WITH (NOLOCK) ' +
                '           WHERE SC_HOGI = 1 ' +
                '           UNION ALL ' +
      ...
      begin
        ...
        if SC_HOGI <> '0' then
        begin
          TEdit(Self.FindComponent('edtJobCnt'+SC_HOGI)).Text := '입고'+IN_CNT+' / '+'출고'+OT_CNT+' / '+'이동'+MV_CNT;
        ...
        begin
          TEdit(Self.FindComponent('edtJobCnt'+SC_HOGI)).Text := '입고'+IN_CNT;
          if (IN_CNT = '0') then
               TEdit(Self.FindComponent('edtJob'+SC_HOGI)).Color := clLime
          else TEdit(Self.FindComponent('edtJob'+SC_HOGI)).Color := $00F2F2F2;
        end;
...
begin
  ...
  try
    ...
    TraceLogWrite( '[260] 자동창고 혼재랙정리 지시 | ' +
                   'OrderProcess -> 작업번호 : ['+OrderData.LUGG+
                   '], 품목코드 : ['+OrderData.ITEM+
                   '], 박스수량 : ['+OrderData.BOX_QTY+
                   '], 출고위치 : ['+OrderData.RACK_MOVE_OT+
                   '], 입고위치 : ['+OrderData.RACK_MOVE_IN+']');
