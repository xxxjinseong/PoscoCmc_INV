...
begin
  ...
  if hlbEhgridListExcel(dgInfo, '출고 실적 조회' + '_' + FormatDatetime('YYYYMMDD', Now)) then
  begin
...
begin
  ...
  // 랩핑여부
  if (Trim(cbWRAP_YN.Text) <> '전체') then
  ...
  try
    ...
    begin
      ...
      StrSQL := ' SELECT H.*, ' +
                ...
                '  WHERE 1 = 1 ' +
                '    AND SUBSTRING(H.ID_TYPE, LEN(H.ID_TYPE)-1, 2) = ''출고'' ' + WhereStr +
                '  ORDER BY H.OR_DT' ;
      SQL.Text := StrSQL;
      Open;
    ...
    begin
      qryInfo.Close;
      TraceLogWrite('[520] 출고이력 조회 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end ;
end;
