...
begin
  ...
  frmU110_Pop.PnlFormName.Caption := '사용자 코드 등록';
  frmU110_Pop.Caption := '사용자 코드 관리';
...
begin
  try
    ...
    begin
      qryInfo.Close;
      TraceLogWrite('[110] 사용자코드 조회 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end ;
end;
...
begin
  try
    ...
    begin
      qryInfoSub.Close;
      TraceLogWrite('[110] 사용자코드 메뉴 조회 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end ;
end;
...
begin
  try
    ...
    begin
      qryInfo.Close;
      TraceLogWrite('[110] 사용자코드 선택 - 에러['+E.Message+']');
    end;
  end;
end;
...
begin
  ...
  frmU110_Pop := TfrmU110_Pop.Create(Application);
  frmU110_Pop.PnlFormName.Caption := '사용자 코드 수정';
  frmU110_Pop.Caption := '사용자 코드 관리';
...
begin
  try
    ...
    begin
      ...
      ExecNo := ExecSQL;
      if ExecNo > 0 then
        TraceLogWrite('[110] 사용자코드 삭제 - 사용자코드['+CODE+'], 로그인계정['+MainDm.M_Info.UserCode+']')
      else
        TraceLogWrite('[110] 사용자코드 삭제 실패 - 사용자코드['+CODE+'], 로그인계정['+MainDm.M_Info.UserCode+']');
      Close;
    end;
    ...
    begin
      ...
      ExecNo := ExecSQL;
      if ExecNo > 0 then
        TraceLogWrite('[110] 사용자코드 메뉴 삭제 - 사용자코드['+CODE+'], 로그인계정['+MainDm.M_Info.UserCode+']')
      else
        TraceLogWrite('[110] 사용자코드 메뉴 삭제 실패 - 사용자코드['+CODE+'], 로그인계정['+MainDm.M_Info.UserCode+']');
      Close;
    end;
  except
    on E : Exception do
    begin
      qryTemp.Close;
      TraceLogWrite('[110] 사용자코드 삭제 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end;
end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        TraceLogWrite('[110] 메뉴권한 업데이트 - 사용자코드['+CODE+
                      '], 메뉴번호['+MenuNo+'], 사용여부['+Used+
                      '], 로그인계정['+MainDm.M_Info.UserCode+']');
      end else
      begin
        TraceLogWrite('[110] 메뉴권한 업데이트 실패 - 사용자코드['+CODE+
                      '], 메뉴번호['+MenuNo+'], 사용여부['+Used+
                      '], 로그인계정['+MainDm.M_Info.UserCode+']');
      end;
      Close;
    ...
    begin
      qryTempSub.Close;
      TraceLogWrite('[110] 메뉴권한 업데이트 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end;
end;
