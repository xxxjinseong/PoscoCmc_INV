...
begin
  ...
  if hlbEhgridListExcel ( dgInfo , '출고 진행현황' + '_' + FormatDatetime('YYYYMMDD', Now) ) then
  begin
...
begin
  try
    ...
    begin
      qryInfo.Close;
      TraceLogWrite('[220] 출고작업 조회 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end;
end;
...
begin
  try
    ...
    begin
      qryInfo.Close;
      TraceLogWrite('[220] 출고작업 선택 - 에러['+E.Message+']');
    end;
  end;
end;
...
begin
  try
    ...
    else if Mode = 3 then TmpType := 'SC완료'
    else if Mode = 4 then TmpType := 'AC완료';

    if (Mode = 3) and (dgInfo.SelectedRows.Count > 1) then
    begin
      MessageDlg('  SC완료 처리는 1건씩 처리 가능합니다.' + #13#10 +
                 '  수동처리 할 작업을 1건 선택 후 진행해 주십시오.', mtConfirmation, [mbYes], 0);
      dgInfo.SetFocus;
      Exit;
    end;

    if (Mode = 3) and (Trim(qryInfo.FieldByName('STATUS').AsString) <> 'SC지시') then
    begin
      MessageDlg('  SC완료 처리는 스태커크레인이 진행중인' + #13#10 +
                 '  작업만 처리 가능합니다.' + #13#10 +
                 '  SC지시 상태인 작업을 선택 후 진행해 주십시오.', mtConfirmation, [mbYes], 0);
      dgInfo.SetFocus;
      Exit;
    end;
    ...
    if (Mode = 4) and (Trim(qryInfo.FieldByName('STATUS').AsString) <> 'AC이동') then
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            OrderProcUpdate('상태수정', ID_DATE, ID_TIME, ID_TYPE, ID_INDEX, 'SC완료');
            OrderProcUpdate('일반삭제', ID_DATE, ID_TIME, ID_TYPE, ID_INDEX, '삭제');
          end;
          TraceLogWrite('[220] 출고작업 수동처리 - 작업번호['+ID_INDEX+'], 처리구분['+TmpType+']');
        end;
      end;
    end else
    //================================================
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            ShowMessage(' 스태커크레인 '+ qryInfo.FieldByName('ID_SC').AsString +' 출고대(' + IntToStr(ID_BUFF) + ')에 데이터가 존재합니다.');
            qryTemp.Close;
            Exit;
          end;
        end;
        ...
        begin
          qryTemp.Close;
          TraceLogWrite('[220] 출고작업 수동처리 - 에러['+E.Message+'], 작업번호['+ID_INDEX+'], 처리구분['+TmpType+'], 쿼리['+StrSQL+']');
        end;
      end;
      OrderProcUpdate('일반수정', ID_DATE, ID_TIME, ID_TYPE, ID_INDEX, 'SC수동완료');
      OrderProcUpdate('상태수정', ID_DATE, ID_TIME, ID_TYPE, ID_INDEX, 'SC완료');
      TraceLogWrite('[220] 출고작업 수동처리 - 작업번호['+ID_INDEX+'], 처리구분['+TmpType+']');
    end else
    //================================================
    ...
    begin
      ...
      try
        ...
        begin
          ...
          if not ( Bof and Eof ) then
          begin
            ShowMessage(' 오토카 하역대(190)에 데이터가 존재합니다.');
            qryTemp.Close;
            Exit;
          end;
        end;
      ...
      //====================================================
      // 데이터가 적재위치(131, 161, 171)에 있는 경우
      //====================================================
      if StrToInt(qryInfo.FieldByName('NOW_POS').AsString) in [131, 161, 171] then
      begin
        TrackDataMove(ID_BUFF, 198); // 적재위치에서 AC로 데이터 이동
        OrderUpdate('AC이동', 'NOW_SITE', IntToStr(198), 198); // 현재위치 업데이트
      end;
      //====================================================
      // 데이터가 오토카(198)에 있는 경우
      //====================================================
      TrackDataMove(198, 190); // AC에서 하역위치로 데이터 이동
      OrderUpdate('AC이동', 'STATUS'  , 'CV이동', 190);
      OrderUpdate('CV이동', 'NOW_MACH', 'CV'    , 190);
      OrderUpdate('CV이동', 'NOW_SITE', IntToStr(190), 190); // 현재위치 업데이트
    ...
    on E : Exception do
    begin
      TraceLogWrite('[220] 출고작업 수동처리 - 에러['+E.Message+'], 작업번호['+ID_INDEX+'], 처리구분['+TmpType+']');
    end;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      TraceLogWrite('[220] 출고작업 수동처리 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end;
end;
