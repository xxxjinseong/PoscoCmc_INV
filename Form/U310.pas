  ...
  try
    ...
    begin
      ...
      StrSQL := ' SELECT RECV_DT as PROC_INFO1 , BODY_02 as PROC_INFO2, BODY_03 as PROC_INFO3, ' +
                ...
                ' CASE WHEN RTRIM(LTRIM(BODY_01)) = ''R'' THEN ''입고예정'' ' +        // 요청구분
                '      WHEN RTRIM(LTRIM(BODY_01)) = ''C'' THEN ''예정취소'' ' +
                '      ELSE '''' END as BODY_01, ' +
                ' CONVERT([varchar](19), CONVERT(DATETIME, (SUBSTRING(RTRIM(LTRIM(BODY_02)), 1, 8) + '' '' + ' +
                ' STUFF(STUFF(SUBSTRING(RTRIM(LTRIM(BODY_02)),9, 6), 3, 0, '':''), 6, 0, '':'')), 21), 21) as BODY_02, ' + // 요청일시
                ' RTRIM(LTRIM(BODY_03)) as BODY_03, ' +                                // 제품번호
                ' RTRIM(LTRIM(BODY_04)) as BODY_04, ' +                                // 제품코드
                ' RTRIM(LTRIM(BODY_05)) as BODY_05, ' +                                // 제품명
                ' CASE WHEN RTRIM(LTRIM(BODY_06)) = '''' or RTRIM(LTRIM(BODY_06)) = NULL THEN CONVERT(NUMERIC(9,3), 0) ' +
                '      ELSE CONVERT(NUMERIC(9,3), RTRIM(LTRIM(BODY_06))) END as BODY_06, ' +  // 중량
                ' CONVERT([varchar](19), CONVERT(DATETIME, (SUBSTRING(RTRIM(LTRIM(BODY_07)), 1, 8) + '' '' + ' +
                ' STUFF(STUFF(SUBSTRING(RTRIM(LTRIM(BODY_07)),9, 6), 3, 0, '':''), 6, 0, '':'')), 21), 21) as BODY_07, ' + // 생산일시
                ' RTRIM(LTRIM(BODY_08)) as BODY_08, ' +                                // 고객코드
                ' RTRIM(LTRIM(BODY_09)) as BODY_09, ' +                                // 고객명
                ' RTRIM(LTRIM(BODY_10)) as BODY_10, ' +                                // 주문번호
                ' RTRIM(LTRIM(BODY_11)) as BODY_11, ' +                                // 공정라인
                ' CASE WHEN RTRIM(LTRIM(BODY_12)) = ''N'' THEN ''반제품'' ' +
                '      WHEN RTRIM(LTRIM(BODY_12)) = ''Y'' THEN ''제품'' ' +
