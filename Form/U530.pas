...
begin
  ...
  if hlbEhgridListExcel(dgInfo, '이동 실적 조회' + '_' + FormatDatetime('YYYYMMDD', Now)) then
  begin
  ...
  try
    ...
    begin
      ...
      StrSQL := ' SELECT H.*, ' +
                ...
                '  WHERE 1 = 1 ' +
                '    AND SUBSTRING(H.ID_TYPE, LEN(H.ID_TYPE)-1, 2) = ''이동'' ' + WhereStr +
                '  ORDER BY H.OR_DT' ;
      SQL.Text := StrSQL;
      Open;
    ...
    begin
      qryInfo.Close;
      TraceLogWrite('[530] 이동이력 조회 - 에러['+E.Message+'], 쿼리['+StrSQL+']');
    end;
  end ;
end;
